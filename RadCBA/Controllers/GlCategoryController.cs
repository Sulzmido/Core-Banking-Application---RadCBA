    ...
    public class GlCategoryController : Controller
    {
        ...
        public ActionResult Edit([Bind(Include = "ID,Name,Description,MainCategory")] GlCategory glCategory)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    // original category is automatically being tracked by the db context. (becos it was read from database, tracked with Unchanged status)
                    // its causing conflict when i track glCategory built by the model binder becos they have the same primary key.
